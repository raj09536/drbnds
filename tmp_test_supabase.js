...
async function testConnection() {
    ...
    const { data: doctors, error: dError } = await supabase.from('doctors').select('id').limit(1)
    if (dError) {
    ...
    const { data: testimonials, error: tError } = await supabase.from('testimonials').select('id').limit(1)
    if (tError) {
